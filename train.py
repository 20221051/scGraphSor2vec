...
# self-defined
from utils import load_data_internal_cell2vec
...
class Trainer:
    def __init__(self, params):
        ...
        self.save_path = self.prj_path / 'pretrained_cell2vec' / f'{self.params.species}' / 'models'
        if not self.save_path.exists():
            self.save_path.mkdir(parents=True)
        self.device = torch.device('cpu' if self.params.gpu == -1 else f'cuda:{params.gpu}')
        self.num_cells, self.num_genes, self.num_labels, self.graph, self.train_ids, self.test_ids, self.labels = load_data_internal_cell2vec(params)
        self.labels = self.labels.to(self.device)
        self.model = GNN(in_feats=self.params.dense_dim+200,
    ...
    def fit(self):
        ...
        acc = final_test_correct_num / len(self.test_ids)
        acc = np.array(acc)
        np.save(f"/home/psy/scDeepSort/acc/train_cell2vec_{self.params.species}_{self.params.tissue}_acc.npy",acc)
    ...
    def train(self):
        ...
        for batch, nf in enumerate(NeighborSampler(g=self.graph,
            ...
            batch_label = batch_nids - self.num_genes
