...
# self-defined
from utils import load_data_cell2vec
...
class Runner:
    def __init__(self, params):
        ...
        if self.params.evaluate:
            self.total_cell, self.num_genes, self.num_classes, self.id2label, self.test_dict, self.map_dict, self.time = load_data_cell2vec(params)
        else:
            self.total_cell, self.num_genes, self.num_classes, self.id2label, self.test_dict, self.time = load_data_cell2vec(params)
        ...
        self.model = GNN(in_feats=params.dense_dim+200,
    ...
    def load_model(self):
        if self.params.species == 'mouse':
            if self.params.tissue == 'Blood':
                model_path = self.prj_path / 'pretrained_cell2vec' / self.params.species / 'models' / f'{self.params.species}-Peripheral_blood.pt'
            elif self.params.tissue == 'Intestine':
                model_path = self.prj_path / 'pretrained_cell2vec' / self.params.species / 'models' / f'{self.params.species}-Small_intestine.pt'
            else:
                model_path = self.prj_path / 'pretrained_cell2vec' / self.params.species / 'models' / f'{self.params.species}-{self.params.tissue}.pt'
        else:
            if self.params.tissue == 'Brain':
                model_path = self.prj_path / 'pretrained_cell2vec' / self.params.species / 'models' / f'{self.params.species}-Cerebellum.pt'
            elif self.params.tissue == 'Blood':
                model_path = self.prj_path / 'pretrained_cell2vec' / self.params.species / 'models' / f'{self.params.species}-Peripheral_blood.pt'
            else:
                model_path = self.prj_path / 'pretrained_cell2vec' / self.params.species / 'models' / f'{self.params.species}-{self.params.tissue}.pt'
...
if __name__ == '__main__':
    ...
    params.threshold = 0
    params.save_dir = 'result_cell2vec'
    pprint(vars(params))
